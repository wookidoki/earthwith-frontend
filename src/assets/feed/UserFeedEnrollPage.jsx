...
import { useNavigate } from 'react-router-dom'; // 1. onGoBack prop 대신 사용
import { Upload, Image, Type, AlignLeft, Save, X, Hash, ChevronLeft } from 'lucide-react';

// src/assets/feed/UserFeedEnrollPage.jsx
const UserFeedEnrollPage = () => {
  ...
  // TODO: (AXIOS) '게시하기' 버튼 클릭 시 폼 데이터 (제목, 내용, 카테고리, 이미지, 태그) API로 전송
  const handleSubmit = () => {
    // 1. 폼 데이터 수집
    // 2. API 전송 (axios.post)
    // 3. 성공 시 피드 페이지로 이동
    alert('게시글이 등록되었습니다. (임시)');
    navigate('/feed'); // 3. 성공 시 피드 페이지로 이동
  };
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div className="flex items-center space-x-2">
             <button 
              onClick={() => navigate(-1)} // 4. '뒤로가기' 기능으로 변경
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            <div className="w-full h-40 rounded-xl border-2 border-dashed border-gray-300 flex flex-col items-center justify-center text-gray-400 hover:border-emerald-400 hover:text-emerald-500 transition-all cursor-pointer bg-gray-50">
              ...
              <span className="text-xs">클릭하여 이미지 업로드 (최대 5장)</span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            <input 
              type="text" 
              className="w-full border-gray-200 rounded-lg py-2 px-4 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-colors placeholder-gray-400"
              placeholder="#태그1 #태그2 (띄어쓰기로 구분)"
            />
          </div>
